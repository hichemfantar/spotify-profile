...
import { logout, useUserInfo } from "../spotify";
		...
		<React.Fragment>
			{userInfoQuery.data?.user ? (
				<Main>
					<Header>
						<Avatar>
							{userInfoQuery.data?.user.images.length > 0 ? (
								<img
									src={userInfoQuery.data?.user.images[0].url}
									alt="avatar"
								/>
							) : (
								<NoAvatar>
						...
						<UserName
							href={userInfoQuery.data?.user.external_urls.spotify}
							target="_blank"
							rel="noopener noreferrer"
						>
							<Name>{userInfoQuery.data?.user.display_name}</Name>
						</UserName>
						<Stats>
							<Stat>
								<Number>{userInfoQuery.data?.user.followers.total}</Number>
								<NumLabel>Followers</NumLabel>
							</Stat>
							{userInfoQuery.data?.followedArtists && (
								<Stat>
									<Number>
										{userInfoQuery.data?.followedArtists.artists.items.length}
									</Number>
					...
					<Preview>
						<Tracklist>
							...
							<div>
								{userInfoQuery.data?.topArtists ? (
									<ul>
										{userInfoQuery.data?.topArtists.items
											.slice(0, 10)
											.map((artist, i) => (
												<Artist key={i}>
						...
						<Tracklist>
							...
							<ul>
								{userInfoQuery.data?.topTracks ? (
									userInfoQuery.data?.topTracks.items
										.slice(0, 10)
